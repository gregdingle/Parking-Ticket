...
def home(request):
    ...
    if request.method == 'POST':
        if 'chance_of_ticket' in request.POST:
            ...
            return render_to_response('home.html', {'TF': ticket_frequency, 'address': address, 'form': adForm},
            ...
            return render_to_response('home.html', {'form': adForm, 'PL': getlaws, 'mvr': most_violated_role,
                                                    'smvr': sec_most_violated_role, 'address': address},
                                      context_instance=RequestContext(request))
...
def getTicketFrequency(location):
    ...
    try:
        P1 = Paths.objects.filter(path__dwithin=(location, 0.0001)).aggregate(end_datetime=Max('end_datetime'),
                                                                              start_datetime=Min('start_datetime'),
                                                                              cnt=Count('id'))
        hours = (mktime(datetime.utctimetuple(P1['end_datetime'])) - mktime(
            datetime.utctimetuple(P1['start_datetime']))) / 3600
